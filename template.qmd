...
Which streets/zip codes have more trees?
...
```{r}
sfo_trees_df |> filter(legal_status %in% c('Permitted Site', 'Undocumented'))
```
...
summarize the counts of oaks and pines
...
```{r}
ggplot(data = oak_pine_df, aes(x=latitude, y=longitude)) +
